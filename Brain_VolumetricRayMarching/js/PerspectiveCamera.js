...
PerspectiveCamera.prototype.move = function(dt, keysPressed) { 
  if(this.isDragging){ 
   ...
   // this.right = this.ahead.cross( new Vec3(0.0, -1.0, 0.0) );
  ...
  Material.shared.eye = this.position;
  Material.shared.viewMtx = this.viewMatrix;
}; 
...
PerspectiveCamera.prototype.apply = function(m){
  m.mul(this.viewMatrix).mul(this.projMatrix);
};
