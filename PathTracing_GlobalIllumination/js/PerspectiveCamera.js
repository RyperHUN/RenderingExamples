...
{
  this.position = new Vec3(3.6, 4.8, 3.5);
  ...
  this.yaw = -0.6;   // Balra jobbra 
  this.pitch = 0.6; //Fel le!
  this.fov = 1.0;
  this.aspect = 1.0;
  this.nearPlane = 0.1;
...
PerspectiveCamera.prototype.move = function(dt, keysPressed) {
  ...
  if(this.isDragging){
   ...
   if(keysPressed.W) {
    this.position.addScaled(this.speed * dt, this.ahead); this.isMoved = true;
  }
  if(keysPressed.S) {
    this.position.addScaled(-this.speed * dt, this.ahead); this.isMoved = true;
  }
  if(keysPressed.D) {
    this.position.addScaled(this.speed * dt, this.right); this.isMoved = true;
  }
  if(keysPressed.A) {
    this.position.addScaled(-this.speed * dt, this.right); this.isMoved = true;
  }
  if(keysPressed.E) {
    this.position.addScaled(this.speed * dt, PerspectiveCamera.worldUp); this.isMoved = true;
  }
  if(keysPressed.Q) {
    this.position.addScaled(-this.speed * dt, PerspectiveCamera.worldUp); this.isMoved = true;
  }
